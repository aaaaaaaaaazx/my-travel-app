...
import { 
  ...
  Luggage, Lock, User, LogIn, UserCog, LogOut, UserPlus, ShieldCheck, Key, ShieldAlert
} from 'lucide-react';

/**
 * 🏆 Travel Planner - 彥麟製作最終黃金基準旗艦版 (2026.02.15)
 * ------------------------------------------------
 * V5.5 旗艦終極穩定版 (Final Stabilized Version)
 * 1. 權限修復：嚴格執行 Rule 3，先 Auth 後 Fetch，解決「權限不足」報錯。
 * 2. 解決 ReferenceError：優化宣告順序，所有子組件均定義在 App 之前。
 * 3. 解決渲染衝突：加固網址偵測邏輯，確保 React Child 節點安全。
 * 4. 管理者通道：登入介面具備顯眼的切換鈕，支援 yljh 管理員與一般使用者。
 * 5. 核心全功能：行程與清單雙向編修、支出統計、匯率互換、圖片網址顯示。
 * 6. 資料保護：路徑鎖定為 'travel-yeh'。
 */

const VERSION_INFO = "旗艦終極穩定版 V5.5 - 2026/02/15 21:55";
...
const CHECKLIST_CATEGORIES = [
  ...
  { id: 'cat_medicine', name: '個人藥品', icon: Pill, items: ['感冒藥', 'OK 繃'] },
...
// --- 子組件：登入視窗 ---
const LoginView = ({ userAuthenticated, onLoginSuccess, onAdminSuccess }) => {
  ...
  // 🔐 Rule 3 保護：等 App 頂層完成 Auth 後才讀取
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        ...
        console.error("LoginView failed to load users", e);
        setIsUsersLoaded(true); // 備援機制
      }
    };
    fetchUsers();
  }, [userAuthenticated]);
  ...
  const handleLogin = (e) => {
    ...
    const found = usersDb.find(u => u.username === acct && u.password === pwd) || (acct === 'abc' && pwd === 'abc');
    if (found) {
      onLoginSuccess();
    } else {
      setErr('帳號或密碼錯誤。');
      ...
      <div className="max-w-md w-full animate-fade-in">
        <div className="text-center mb-10">
          <div className="w-20 h-20 bg-blue-600 text-white rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-2xl rotate-12 shadow-blue-200">
            {isAdminMode ? <ShieldCheck size={40} /> : <Plane size={40} />}
          </div>
          <h2 className="text-3xl font-black text-slate-900 tracking-tight">{isAdminMode ? '管理者控制台' : '旅程規劃系統'}</h2>
          <p className="text-slate-400 font-bold mt-2">{isAdminMode ? '請輸入管理員憑證' : '為您的冒險量身打造智能管家'}</p>
        </div>

        <form onSubmit={handleLogin} className="bg-white p-10 rounded-[3rem] shadow-2xl border border-slate-200 space-y-6">
          ...
          <button type="submit" disabled={!isUsersLoaded} className="w-full bg-blue-600 hover:bg-blue-700 text-white py-5 rounded-2xl font-black flex items-center justify-center gap-3 shadow-xl active:scale-95 disabled:opacity-50">
            {!isUsersLoaded ? <Loader2 className="animate-spin" /> : <><LogIn size={20} /> 立即進入系統</>}
          </button>
        </form>

        <div className="mt-8 flex flex-col items-center gap-4">
          <button onClick={() => { setIsAdminMode(!isAdminMode); setErr(''); }} className="flex items-center gap-2 text-xs font-black text-blue-600 bg-blue-50 px-8 py-3 rounded-full hover:bg-blue-100 transition-all shadow-sm">
            {isAdminMode ? <User size={14}/> : <UserCog size={14}/>}
            {isAdminMode ? '返回一般登入入口' : '切換至管理員通道'}
          </button>
...
const AdminDashboard = ({ user, onLogout }) => {
  ...
  if (loading) return <div className="flex flex-col items-center justify-center h-screen"><Loader2 className="animate-spin text-blue-600 mb-4" size={48} /><p className="font-bold text-slate-400">系統授權認證中...</p></div>;
    ...
    <div className="w-full min-h-screen bg-slate-50 p-6 md:p-12 animate-fade-in space-y-12 font-sans">
      <header className="max-w-5xl mx-auto flex justify-between items-center bg-slate-900 text-white p-8 rounded-[3rem] shadow-2xl">
        <div className="flex items-center gap-4">
          ...
          <div><h2 className="text-2xl font-black tracking-tight">後台帳號管理系統</h2><p className="text-slate-400 text-xs font-bold">Admin Level Access</p></div>
        </div>
      ...
      <div className="max-w-5xl mx-auto bg-white p-8 md:p-10 rounded-[4rem] shadow-xl border border-slate-100">
        <h3 className="text-lg font-black text-slate-800 mb-6 flex items-center gap-2"><UserPlus className="text-blue-600" /> {editingUser ? `重設使用者 ${editingUser.username} 的密碼` : '建立新使用者帳號'}</h3>
      ...
      <div className="max-w-5xl mx-auto bg-white rounded-[3rem] shadow-xl border border-slate-100 overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-slate-50 text-slate-400 text-[10px] font-black uppercase tracking-widest">
            <tr><th className="px-8 py-5">使用者帳號</th><th className="px-8 py-5">登入密碼</th><th className="px-8 py-5">建立日期</th><th className="px-8 py-5 text-center">操作</th></tr>
          </thead>
            ...
            {users.map(u => (
              <tr key={u.id} className="hover:bg-slate-50 transition-colors group">
                ...
                <td className="px-8 py-5 text-center">
                  <div className="flex justify-center gap-2"><button onClick={() => { setEditingUser(u); setNewPwd(u.password); }} className="p-2 text-slate-300 hover:text-blue-600 transition-colors"><Edit3 size={18}/></button><button onClick={async () => { if(confirm('刪除帳號？')) await deleteDoc(doc(fDb, 'artifacts', fAppId, 'public', 'data', 'users_db', u.id)); }} className="p-2 text-slate-300 hover:text-red-500 transition-colors"><Trash2 size={18}/></button></div>
                </td>
              </tr>
            ))}
          </tbody>
  ...
  <div className="space-y-12">
    ...
    <div className="bg-white p-6 md:p-12 rounded-[4rem] shadow-sm border border-slate-100">
      <form onSubmit={async e => { e.preventDefault(); const current = props.itineraryData?.days?.[props.activeDay]?.spots || []; await props.updateItinField(`days.${props.activeDay}.spots`, [...current, { ...props.newSpot, id: Date.now().toString() }]); props.setNewSpot({ time: '09:00', spot: '', note: '', imageUrl: '' }); }} className="mb-12 space-y-4 bg-slate-50 p-6 rounded-[2.5rem] border border-slate-100 shadow-inner">
        <div className="flex gap-3 flex-wrap md:flex-nowrap"><div className="flex items-center gap-2 bg-white px-4 py-2 rounded-xl border w-full md:w-auto shadow-sm shadow-inner"><Clock size={18} className="text-blue-500" /><input type="time" value={props.newSpot.time} onChange={e => props.setNewSpot({...props.newSpot, time: e.target.value})} className="bg-transparent font-black outline-none w-24" /></div><input placeholder="想在那裡留下足跡？" required value={props.newSpot.spot} onChange={e => props.setNewSpot({...props.newSpot, spot: e.target.value})} className="flex-1 p-3 bg-white border rounded-xl font-bold outline-none shadow-sm shadow-inner" /></div>
      ...
      <div className="space-y-8 px-2 relative before:content-[''] before:absolute before:left-[35px] before:top-4 before:bottom-4 before:w-1.5 before:bg-slate-50 before:rounded-full">
          ...
          return (
            <div key={item.id} className="relative pl-20 group">
              <div className="absolute left-[-15px] top-1/2 -translate-y-1/2 flex flex-col items-center gap-1">
                <button onClick={(e) => { e.stopPropagation(); const spots = [...props.itineraryData.days[props.activeDay].spots]; if (idx > 0) { [spots[idx], spots[idx-1]] = [spots[idx-1], spots[idx]]; props.updateItinField(`days.${props.activeDay}.spots`, spots); } }} className="text-slate-200 hover:text-blue-600 transition-colors"><ArrowUp size={20}/></button>
              ...
              <div onClick={() => !isEditing && props.setExpandedItems(prev => ({...prev, [item.id]: !prev[item.id]}))} className={`p-10 bg-white border rounded-[3rem] transition-all cursor-pointer ${isEditing ? 'border-blue-600 shadow-2xl ring-8 ring-blue-50' : 'border-slate-100 hover:shadow-2xl shadow-sm'}`}>
                {isEditing ? ( 
                  <div className="space-y-4 animate-fade-in" onClick={e => e.stopPropagation()}>
                    ...
                    <input placeholder="圖片網址" value={props.editData.imageUrl || ''} onChange={e => props.setEditData({...props.editData, imageUrl: e.target.value})} className="w-full p-3 border rounded-xl bg-slate-50 outline-none text-xs font-bold shadow-inner" />
                    <textarea value={props.editData.note} onChange={e => props.setEditData({...props.editData, note: e.target.value})} className="w-full p-3 border rounded-xl h-24 bg-slate-50 outline-none shadow-inner text-sm" />
                    <div className="flex justify-end gap-3"><button onClick={(e) => { e.stopPropagation(); props.setEditingId(null); }} className="text-sm font-bold text-slate-400 px-4">取消</button><button onClick={async (e) => { e.stopPropagation(); const spots = props.itineraryData.days[props.activeDay].spots.map(s => s.id === props.editingId ? props.editData : s); await props.updateItinField(`days.${props.activeDay}.spots`, spots); props.setEditingId(null); }} className="bg-blue-600 text-white px-6 py-2 rounded-xl text-sm font-black shadow-lg">儲存變更</button></div>
                  </div>
...
const App = () => {
  ...
  useEffect(() => {
    ...
    initAppAuth();
    const unsubscribe = onAuthStateChanged(fAuth, (u) => { if (u) setUser(u); });
    return () => unsubscribe();
  }, []);
  ...
  useEffect(() => {
    ...
    }, (err) => { if (err.code === 'permission-denied') console.error("Trips Snap Error", err); });
      ...
      {view === 'home' ? (
        ...
        <div className="w-full flex flex-col items-center pb-24 animate-fade-in px-4">
          ...
          <main className="w-full max-w-5xl p-4 md:p-12">
            ...
            ) : activeTab === 'weather' ? <WeatherMasterSection tripInfo={tripInfo} /> : activeTab === 'expenses' ? <ExpenseMasterSection itineraryData={itineraryData} updateItinField={updateItinField} /> : activeTab === 'checklist' ? <ChecklistMasterSection itineraryData={itineraryData} updateItinField={updateItinField} /> : <CurrencyMaster itineraryData={itineraryData} updateItinField={updateItinField} /> }
          </main>
