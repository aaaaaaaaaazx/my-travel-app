 ...
 * ------------------------------------------------
 * V7.5 手機響應式加固版：
 * 1. 修復手機位移：全面優化 Grid 與 Flex 佈局，確保手機端位置不跑掉。
 * 2. 忠於原稿：完全還原 0215a.txt 的天氣、匯率、清單與費用組件。
 * 3. 帳號同步：Admin 進入後台自動補齊 abc 帳號至雲端。
 * 4. 權限修復：嚴格執行 Rule 3 (Auth -> Fetch)。
 * 5. 解決空白問題：確保組件定義順序與分頁呼叫名稱完全匹配。
 */

const VERSION_INFO = "旗艦終極穩定版 V7.5 - 2026/02/16 01:15";
...
// --- 子組件：費用、天氣、匯率、清單 (由原稿 0215a.txt 恢復) ---
    ...
    <div className="w-full flex flex-col items-center min-h-screen text-slate-700">
      ...
      {view === 'home' ? (
        ...
        <div className="w-full flex flex-col items-center pb-24 animate-fade-in">
          <nav className="w-full h-20 bg-white/90 backdrop-blur-xl border-b border-slate-100 flex items-center justify-between px-4 sm:px-10 sticky top-0 z-50 shadow-sm max-w-[100vw] overflow-x-hidden">
            <div className="font-black text-blue-600 text-xl sm:text-2xl flex items-center gap-2 sm:gap-3 cursor-pointer group" onClick={() => window.location.reload()}><Plane size={24} className="rotate-45" /><span className="tracking-tighter uppercase font-black font-sans hidden xs:inline">Traveler</span></div>
            ...
            <div className="text-right flex items-center gap-3">
              <div className="text-right"><div className="font-black text-slate-800 tracking-tight truncate max-w-[120px] sm:max-w-[180px] leading-none text-sm sm:text-base">{tripInfo.city} 之旅</div><div className="text-[9px] font-bold text-slate-400 mt-1 uppercase">{tripInfo.startDate}</div></div>
