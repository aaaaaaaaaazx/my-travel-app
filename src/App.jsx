 ...
 * ------------------------------------------------
 * V5.9 樣式與權限終極優化版：
 * 1. CSS 穩定修復：重新設計樣式注入系統，確保 Tailwind 與自訂 CSS 完全生效。
 * 2. 權限加固：嚴格執行 Rule 3，確保先建立匿名 Auth 連線才發起資料請求。
 * 3. 管理者介面：提供清晰的切換入口，支援管理員 yljh 管理帳號。
 * 4. 全功能整合：行程與清單雙向編修、支出統計、匯率互換、圖片網址支援。
 * 5. 資料傳承：鎖定 fAppId = 'travel-yeh'，找回原有資料。
 */

const VERSION_INFO = "旗艦穩定版 V5.9 - 2026/02/15 22:55";
...
const LoginView = ({ authUser, onLoginSuccess, onAdminSuccess }) => {
  ...
  useEffect(() => {
    ...
    const loadUsers = async () => {
      if (!authUser) return; // 守衛：Auth 準備好才發請求
      try {
        ...
        console.warn("Login fetch restricted, static mode active.");
    ...
    <div className="min-h-screen w-full bg-slate-50 flex items-center justify-center p-6 font-sans">
      <div className="max-w-md w-full animate-fade-in">
        <div className="text-center mb-8">
          ...
          <p className="text-slate-400 font-bold mt-2">{isAdminPortal ? '系統管理與安全性控制' : '為您的冒險量身打造智能管家'}</p>
        </div>
        ...
        <div className="mt-8 flex flex-col items-center gap-4">
          <button onClick={() => { setIsAdminPortal(!isAdminPortal); setErr(''); }} className="flex items-center gap-2 text-xs font-black text-blue-600 bg-blue-50 px-8 py-3 rounded-full hover:bg-blue-100 transition-all shadow-sm">
            ...
            {isAdminPortal ? '返回一般使用者登入' : '切換至管理者入口'}
          </button>
...
const AdminDashboard = ({ authUser, onLogout }) => {
  ...
  if (loading) return <div className="flex flex-col items-center justify-center p-20 h-screen"><Loader2 className="animate-spin text-blue-600 mb-4" size={48} /><p className="font-bold text-slate-400">正在建立加密連線...</p></div>;
    ...
    <div className="w-full min-h-screen bg-slate-50 p-6 md:p-12 animate-fade-in space-y-12">
      <header className="max-w-5xl mx-auto flex justify-between items-center bg-slate-900 text-white p-8 rounded-[3rem] shadow-2xl">
        <div className="flex items-center gap-4">
          ...
          <div><h2 className="text-2xl font-black tracking-tight">管理中心</h2><p className="text-slate-400 text-xs font-bold uppercase tracking-widest">Admin Access</p></div>
        </div>
      ...
      <div className="max-w-5xl mx-auto bg-white p-8 md:p-10 rounded-[4rem] shadow-xl border border-slate-100">
        <h3 className="text-lg font-black text-slate-800 mb-6 flex items-center gap-2"><UserPlus size={20} className="text-blue-600" /> {editingUser ? `編輯: ${editingUser.username}` : '建立新帳號'}</h3>
        <form onSubmit={handleSave} className="flex flex-wrap md:flex-nowrap gap-4 items-end">
          ...
          <div className="flex gap-2">{editingUser && <button type="button" onClick={() => {setEditingUser(null); setNewPwd('');}} className="px-6 py-4 rounded-2xl font-bold text-slate-400">取消</button>}<button type="submit" className="bg-blue-600 text-white px-8 py-4 rounded-2xl font-black shadow-xl active:scale-95">{editingUser ? '儲存修改' : '立即建立'}</button></div>
        </form>
      </div>
      <div className="max-w-5xl mx-auto bg-white rounded-[3rem] shadow-xl border border-slate-100 overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-slate-50 text-slate-400 text-[10px] font-black uppercase tracking-widest">
            <tr><th className="px-8 py-5">使用者</th><th className="px-8 py-5">密碼</th><th className="px-8 py-5 text-center">操作</th></tr>
          </thead>
...
// --- 子組件：費用、天氣、匯率、清單、行程 ---
const ExpenseSection = ({ itineraryData, updateItinField }) => {
              ...
              <div onClick={() => !isEditing && props.setExpandedItems(prev => ({...prev, [item.id]: !prev[item.id]}))} className={`p-10 bg-white border rounded-[3rem] transition-all cursor-pointer ${isEditing ? 'border-blue-600 shadow-2xl ring-8 ring-blue-50' : 'border-slate-100 hover:shadow-2xl shadow-sm'}`}>
                {isEditing ? ( 
                  <div className="space-y-4 animate-fade-in" onClick={e => e.stopPropagation()}>
                    <div className="flex gap-2"><input type="time" value={editData.time} onChange={e => props.setEditData({...props.editData, time: e.target.value})} className="p-3 border rounded-xl font-black w-32 bg-slate-50 outline-none shadow-inner" /><input value={props.editData.spot} onChange={e => props.setEditData({...props.editData, spot: e.target.value})} className="flex-1 p-3 border rounded-xl font-black bg-slate-50 outline-none shadow-inner" /></div>
...
const App = () => {
  ...
  useEffect(() => {
    ...
    const style = document.createElement('style'); style.id = 'premium-ui-engine-v5.9';
    style.innerHTML = `
      ...
      {view === 'home' ? (
        ...
        <div className="w-full flex flex-col items-center pb-24 animate-fade-in px-4">
          ...
          <main className="w-full max-w-5xl p-4 md:p-12">
            ...
            ) : activeTab === 'weather' ? <WeatherMaster tripInfo={tripInfo} /> : activeTab === 'expenses' ? <ExpenseMasterSection itineraryData={itineraryData} updateItinField={updateItinField} /> : activeTab === 'checklist' ? <ChecklistSection itineraryData={itineraryData} updateItinField={updateItinField} /> : <CurrencyMasterSection itineraryData={itineraryData} updateItinField={updateItinField} /> }
          </main>
