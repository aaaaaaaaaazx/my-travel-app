...
/**
 * 🚀 全功能整合修復版 (2026.02.05):
 * 1. 天氣預測升級：支援手動輸入國家、城市、日期進行 Google 搜尋。
 * 2. 修復 ReferenceError: 確保 handleCreate 函數定義完整。
 * 3. AI 安全性：遵守 apiKey = "" 規範，由環境自動注入。
 * 4. 行程編修：支援景點原位修改與靈敏排序。
 */

const VERSION_INFO = "最後更新：2026/02/05 10:45 (天氣自訂查詢版)";
...
const appId = typeof __app_id !== 'undefined' ? __app_id : 'travel-planner-app';

// 系統規範：apiKey 必須為空，由環境注入
const apiKey = ""; 
...
const App = () => {
  ...
  useEffect(() => {
    ...
    style.innerHTML = `
      html, body, #root { min-height: 100%; width: 100%; margin: 0; padding: 0; background-color: #f8fafc; font-family: sans-serif; }
  ...
  const callGemini = async (userQuery, isJson = false) => {
    ...
    try {
      const payload = { 
        ...
        systemInstruction: { 
          parts: [{ text: isJson ? "Output valid JSON only. No markdown." : "Helpful travel assistant." }] 
        },
        tools: [{ "google_search": {} }],
      ...
      <div className="animate-fade-in max-w-4xl mx-auto w-full">
        <div className="flex gap-3 overflow-x-auto pb-6 mb-8 scrollbar-hide">
          {Object.keys(itineraryData.days).map(day => (
            <button key={day} onClick={() => {setActiveDay(parseInt(day)); setEditingId(null);}} className={`shrink-0 px-8 py-4 rounded-2xl font-black border transition-all ${activeDay === parseInt(day) ? 'bg-blue-600 text-white shadow-xl scale-105 border-blue-600' : 'bg-white text-slate-400 border-slate-100 hover:bg-slate-50'}`}>
        ...
        <div className="bg-white p-8 md:p-12 rounded-[3.5rem] shadow-sm border border-slate-100">
          <div className="flex justify-between items-end mb-10">
            <div><h2 className="text-5xl font-black text-slate-900 italic tracking-tighter">Day {activeDay}</h2><p className="text-slate-400 font-bold uppercase text-[10px] mt-2 tracking-widest">{getFormattedDate(tripInfo.startDate, activeDay)}</p></div>
                ...
                <div className={`p-8 bg-white border rounded-[2.5rem] flex justify-between items-start transition-all shadow-sm ${editingId === item.id ? 'border-blue-500 ring-8 ring-blue-50/50 shadow-2xl' : 'border-slate-100 hover:shadow-xl border-l-8 border-l-transparent hover:border-l-blue-600'}`}>
                  {editingId === item.id ? (
                    <div className="space-y-4 flex-1 animate-fade-in">
                       ...
                       <div className="flex justify-end gap-3"><button onClick={() => setEditingId(null)} className="text-sm font-bold text-slate-400 px-4">取消</button><button onClick={async () => { const updated = currentDay.spots.map(s => s.id === editingId ? editData : s); await updateItinField(`days.${activeDay}.spots`, updated); setEditingId(null); }} className="bg-blue-600 text-white px-6 py-2 rounded-xl text-sm font-black flex items-center gap-2 shadow-lg hover:bg-blue-700 transition-colors"><Save size={16}/> 儲存</button></div>
                    </div>
                  ) : (
                    <>
                      <div className="space-y-3 flex-1">
                        <div className="flex items-center gap-3"><h4 className="text-2xl font-black text-slate-800 tracking-tight">{item.spot}</h4><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(item.spot)}`} target="_blank" rel="noreferrer" className="p-1.5 bg-blue-50 text-blue-600 rounded-lg text-[10px] font-black flex items-center gap-1 hover:bg-blue-600 hover:text-white transition-all shadow-sm"><MapPin size={12}/> 地圖</a></div>
                      ...
                      <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all ml-4">
                        <button onClick={() => { setEditingId(item.id); setEditData({ ...item }); }} className="p-3 text-slate-300 hover:text-blue-500 bg-slate-50 rounded-2xl transition-transform active:scale-90"><Edit3 size={18}/></button>
                        <button onClick={async () => { await updateItinField(`days.${activeDay}.spots`, currentDay.spots.filter(s => s.id !== item.id)); }} className="p-3 text-slate-300 hover:text-red-500 bg-slate-50 rounded-2xl transition-transform active:scale-90"><Trash2 size={18}/></button>
                      </div>
                    </>
                  )}
                </div>
      ...
      <div className="max-w-4xl mx-auto space-y-6 animate-fade-in w-full">
        <div className="bg-white p-10 rounded-[3.5rem] shadow-sm border border-slate-100">
          ...
          <div className="grid grid-cols-2 gap-4 mb-10">
            <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">去程日期</label><input type="date" value={fInfo.departDate} onChange={e => updateItinField('flightsInfo.departDate', e.target.value)} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none font-bold" /></div>
            <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">回程日期</label><input type="date" value={fInfo.returnDate} onChange={e => updateItinField('flightsInfo.returnDate', e.target.value)} className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none font-bold" /></div>
          </div>
          <div className="space-y-4 mb-10">
            {fInfo.flights?.map(f => (
              <div key={f.id} className="p-6 bg-slate-50 rounded-3xl border border-slate-100 flex justify-between items-center group relative overflow-hidden transition-all"><div className="flex items-center gap-4 relative z-10"><div className="bg-blue-600 text-white p-3 rounded-2xl shadow-lg shadow-blue-100"><PlaneTakeoff size={20}/></div><div><div className="flex items-center gap-3"><span className="text-xl font-black text-slate-800">{f.flightNo}</span><span className="bg-white px-2 py-0.5 rounded-full text-[9px] font-black border border-slate-200 text-slate-400 uppercase">{f.type}</span></div><p className="text-xs text-slate-400 font-bold mt-1">{f.time}</p></div></div><button onClick={async () => { await updateItinField(`flightsInfo.flights`, fInfo.flights.filter(fl => fl.id !== f.id)); }} className="p-3 text-slate-300 hover:text-red-500 bg-white rounded-2xl shadow-sm transition-all opacity-0 group-hover:opacity-100"><Trash2 size={18}/></button></div>
            ))}
          </div>
          <form onSubmit={async (e) => { e.preventDefault(); await updateItinField(`flightsInfo.flights`, [...(fInfo.flights || []), { ...newF, id: Date.now().toString() }]); setNewF({ flightNo: '', time: '08:00', type: '去程' }); }} className="flex gap-3 bg-slate-900 p-5 rounded-[2.5rem] shadow-xl"><input required placeholder="航班號 (如: BR198)" value={newF.flightNo} onChange={e => setNewF({...newF, flightNo: e.target.value.toUpperCase()})} className="flex-1 p-3 rounded-2xl bg-white/10 text-white placeholder-white/30 border-none outline-none font-black text-sm" /><input type="time" value={newF.time} onChange={e => setNewF({...newF, time: e.target.value})} className="p-3 rounded-2xl bg-white/10 text-white border-none outline-none font-black text-sm w-32" /><button type="submit" className="bg-blue-600 text-white px-8 rounded-2xl font-black hover:bg-blue-500 active:scale-95 transition-all">新增</button></form>
        </div>
      </div>
    );
  };
  ...
  const WeatherView = () => {
    const [queryInfo, setQueryInfo] = useState({ country: tripInfo.country, city: tripInfo.city, date: tripInfo.startDate });
    const [weatherResult, setWeatherResult] = useState(null);

    const fetchWeather = async (e) => {
        if(e) e.preventDefault();
        const prompt = `查「${queryInfo.country} ${queryInfo.city}」在「${queryInfo.date}」的天氣預報資訊。輸出 JSON 且無 Markdown：{"temp": "氣溫", "condition": "狀態", "tips": "穿衣建議"}`;
        const res = await callGemini(prompt, true);
        if (res) {
            try {
                const data = JSON.parse(res);
                setWeatherResult(data);
                // 同時儲存至目前旅程的對應天數中 (如果是選定的旅程日期)
                if (queryInfo.city === tripInfo.city) {
    ...
    return (
      <div className="animate-fade-in max-w-4xl mx-auto w-full space-y-6">
        <div className="bg-white p-10 rounded-[3.5rem] shadow-sm border border-slate-100">
            <h3 className="text-2xl font-black mb-8 flex items-center gap-2"><Sun className="text-yellow-500"/> 自訂天氣查詢</h3>
            <form onSubmit={fetchWeather} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                <div className="space-y-1">
                    ...
                    <input required value={queryInfo.country} onChange={e => setQueryInfo({...queryInfo, country: e.target.value})} className="w-full p-3 bg-slate-50 border rounded-xl font-bold outline-none focus:ring-2 focus:ring-blue-500/10" />
                </div>
                <div className="space-y-1">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">城市</label>
                    <input required value={queryInfo.city} onChange={e => setQueryInfo({...queryInfo, city: e.target.value})} className="w-full p-3 bg-slate-50 border rounded-xl font-bold outline-none focus:ring-2 focus:ring-blue-500/10" />
                </div>
                <div className="space-y-1">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">日期</label>
                    <input required type="date" value={queryInfo.date} onChange={e => setQueryInfo({...queryInfo, date: e.target.value})} className="w-full p-3 bg-slate-50 border rounded-xl font-bold outline-none focus:ring-2 focus:ring-blue-500/10" />
                </div>
                <button type="submit" disabled={aiLoading} className="bg-blue-600 text-white h-[50px] rounded-xl font-black flex items-center justify-center gap-2 hover:bg-blue-700 transition-all active:scale-95 disabled:opacity-50">
        ...
        {weatherResult && (
            <div className="bg-white p-12 rounded-[4rem] shadow-xl border border-blue-50 text-center relative overflow-hidden animate-fade-in">
                <div className="absolute top-0 right-0 p-10 text-blue-50/30 -z-10"><Cloud size={200}/></div>
                <div className="space-y-6">
                    <p className="text-slate-400 font-bold uppercase tracking-widest text-xs">{queryInfo.city} · {queryInfo.date} 預報結果</p>
                    <div className="text-8xl font-black text-slate-900 tracking-tighter leading-none">{weatherResult.temp}</div>
                    <div className="text-2xl font-black text-blue-600">{weatherResult.condition}</div>
                    <div className="bg-blue-50 p-8 rounded-[3rem] border border-blue-100 max-w-md mx-auto shadow-sm">
                        <p className="text-blue-700 font-bold text-sm leading-relaxed whitespace-pre-wrap">{weatherResult.tips}</p>
                    </div>
                </div>
            </div>
        )}
  ...
  const CurrencyView = () => {
    ...
    const fetchRate = async () => {
      const prompt = `查台幣 TWD 兌換「${tripInfo.country}」當地貨幣匯率。輸出 JSON：{"rate": 匯率, "currencyName": "名稱", "tips": "建議"}`;
    ...
    return (
      <div className="bg-white p-12 rounded-[4rem] shadow-sm border border-slate-100 text-center animate-fade-in max-w-4xl mx-auto w-full">
        ...
        {info ? (
          <div className="space-y-8 animate-fade-in"><div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center max-w-2xl mx-auto"><div className="p-8 bg-slate-50 rounded-[3rem] border border-slate-100 shadow-inner"><p className="text-[10px] font-black text-slate-400 mb-2 uppercase tracking-widest text-center italic">TWD 台幣</p><input type="number" value={amount} onChange={e => setAmount(e.target.value)} className="bg-transparent text-5xl font-black w-full text-center outline-none text-slate-800" /></div><div className="p-8 bg-blue-600 text-white rounded-[3rem] shadow-2xl flex flex-col items-center justify-center animate-fade-in shadow-blue-200"><p className="text-[10px] font-black opacity-60 mb-2 tracking-widest uppercase text-center italic">{info.currencyName}</p><div className="text-5xl font-black truncate w-full">{Number(parseFloat(amount) * parseFloat(info.rate)).toLocaleString(undefined, {maximumFractionDigits: 2})}</div><p className="text-[10px] mt-4 opacity-80 font-black bg-white/20 px-4 py-1 rounded-full text-center tracking-wider">Rate: 1 TWD = {info.rate}</p></div></div><div className="bg-slate-50 p-8 rounded-[3rem] border border-slate-100 text-left italic text-sm text-slate-500 font-bold leading-relaxed shadow-sm shadow-inner"><Sparkles size={16} className="text-blue-500 mb-2 inline mr-2"/> {info.tips}</div><button onClick={fetchRate} className="text-slate-300 text-xs font-bold underline hover:text-blue-600 transition-colors">重新獲取匯率</button></div>
        ) : (
          <div className="py-10"><Coins className="text-blue-50 mx-auto mb-6" size={100} /><p className="text-slate-400 font-bold mb-8 uppercase tracking-widest text-xs text-center italic">正在搜尋匯率資訊...</p><button onClick={fetchRate} disabled={aiLoading} className="bg-blue-600 text-white px-10 py-5 rounded-[2rem] font-black shadow-xl flex items-center gap-3 mx-auto hover:scale-105 active:scale-95 transition-all">{aiLoading ? <Loader2 className="animate-spin" size={24}/> : <Coins size={24}/>} 查詢即時匯率</button></div>
        )}
      </div>
    );
  };
