...
/**
 * 🚀 全功能穩定整合版 (2026.02.05):
 * 1. 天氣預測：整合 Open-Meteo 地理編碼與氣象 API，支援目的地與日期範圍查詢。
 * 2. 權限修復：嚴格遵守身份驗證後監聽規則，固定 appId 為 'travel-yeh'。
 * 3. 匯率與清單：完整保留所有自定義與管理功能。
 */

const VERSION_INFO = "最後更新：2026/02/05 17:15 (氣象 API 整合版)";
    ...
    <div className="animate-fade-in space-y-10 w-full max-w-5xl mx-auto">
      <div className="bg-white p-10 rounded-[4rem] shadow-xl border border-slate-100">
        <h3 className="text-2xl font-black text-slate-800 mb-8 flex items-center gap-3">
          <Sun className="text-orange-500" /> 全球精準天氣預測
        </h3>
        <form onSubmit={fetchWeather} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">旅遊目的地</label>
            <div className="relative">
              <MapPin className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={18}/>
              <input required value={q.dest} onChange={e => setQ({...q, dest: e.target.value})} className="w-full pl-12 pr-4 py-4 bg-slate-50 border-2 border-transparent focus:border-blue-500 rounded-3xl outline-none font-bold shadow-inner" placeholder="如：富國島" />
            </div>
          </div>
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">開始日期</label>
            <input required type="date" value={q.start} onChange={e => setQ({...q, start: e.target.value})} className="w-full p-4 bg-slate-50 border-2 border-transparent focus:border-blue-500 rounded-3xl outline-none font-bold shadow-inner" />
          </div>
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">結束日期</label>
            <input required type="date" value={q.end} min={q.start} onChange={e => setQ({...q, end: e.target.value})} className="w-full p-4 bg-slate-50 border-2 border-transparent focus:border-blue-500 rounded-3xl outline-none font-bold shadow-inner" />
          </div>
          <button type="submit" disabled={loading} className="bg-blue-600 text-white h-[60px] rounded-3xl font-black shadow-lg hover:bg-blue-700 active:scale-95 transition-all flex items-center justify-center gap-2">
            {loading ? <Loader2 className="animate-spin" /> : <Search size={20}/>} 搜尋氣象
          </button>
        </form>
      ...
      {results && (
        <div className="space-y-6">
          <div className="flex items-end gap-4 ml-4">
            <h4 className="text-4xl font-black text-slate-900 tracking-tighter italic">{results.location}</h4>
            <span className="text-slate-400 font-bold text-sm mb-1 uppercase tracking-widest">Weather Forecast</span>
          </div>
              ...
              return (
                <div key={day.date} className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-lg hover:shadow-2xl transition-all group">
                  ...
                  <div className="flex justify-between items-start mb-6">
                    <info.icon className={`${info.color} group-hover:scale-110 transition-transform`} size={48} />
                    <div className="text-right">
                       <p className="text-3xl font-black text-slate-800">{Math.round(day.max)}°</p>
                       <p className="text-sm font-bold text-slate-300">{Math.round(day.min)}°</p>
                    </div>
                  </div>
                  <div className="bg-slate-50 p-4 rounded-2xl">
                    <p className={`font-black text-sm mb-1 ${info.color}`}>{info.label}</p>
                    <p className="text-[11px] text-slate-500 font-bold leading-relaxed">{info.tip}</p>
                  </div>
                </div>
              );
            })}
...
const CurrencyMaster = () => {
  ...
  const fetchRates = async () => {
    ...
    } catch (err) { setError('連線失敗。'); } finally { setLoading(false); }
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-7 gap-8 items-center">
            <div className="md:col-span-3">
              ...
              <div className="relative">
                ...
                <div className="absolute right-4 top-1/2 -translate-y-1/2">
                  <select value={baseCurrency} onChange={e => setBaseCurrency(e.target.value)} className="bg-white border shadow-sm rounded-xl px-3 py-1.5 text-xs font-black cursor-pointer outline-none">
            ...
            <div className="md:col-span-3">
              ...
              <div className="w-full pl-8 pr-6 py-5 bg-blue-600 rounded-[2rem] text-white flex items-center justify-between shadow-xl shadow-blue-100">
                <div><span className="text-3xl font-black">{convertedAmount}</span><p className="text-blue-100 text-[10px] mt-1 font-bold">{targetCurrency}</p></div>
                <select value={targetCurrency} onChange={e => setTargetCurrency(e.target.value)} className="bg-blue-700 text-white border-none rounded-xl px-3 py-1.5 text-xs font-black cursor-pointer outline-none">
                  {Object.keys(rates).map(curr => <option key={curr} value={curr}>{curr}</option>)}
                </select>
              </div>
            </div>
      ...
      {showSettings && (
        <div className="bg-white rounded-[3rem] border border-slate-100 shadow-2xl overflow-hidden animate-fade-in">
          <div className="p-8 border-b flex justify-between items-center bg-slate-50/50">
            <h3 className="font-black text-xl text-slate-800">國家匯率自定義</h3>
            <div className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={18} /><input type="text" placeholder="搜尋..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-12 pr-6 py-3 border rounded-2xl text-sm font-bold outline-none focus:ring-4 ring-blue-50 w-64" /></div>
          </div>
          <div className="max-h-[400px] overflow-y-auto scrollbar-hide">
            <table className="w-full text-left">
              <thead className="bg-slate-50 text-slate-400 text-[10px] uppercase font-black sticky top-0 z-10"><tr><th className="px-8 py-5">國家與幣別</th><th className="px-8 py-5 text-right">市場</th><th className="px-8 py-5 text-center">模式</th><th className="px-8 py-5 text-right">數值</th></tr></thead>
              <tbody className="divide-y divide-slate-50">
                {filteredCurrencies.map(curr => (
                  <tr key={curr} className="hover:bg-blue-50/30 transition-colors">
                    <td className="px-8 py-5"><div className="font-black text-slate-700">{currencyNames[curr] || "其他"}</div><div className="text-[10px] text-slate-400 font-bold uppercase">{curr}</div></td>
                    <td className="px-8 py-5 text-right font-mono text-slate-500 text-sm font-bold">{rates[curr]?.toFixed(4)}</td>
                    <td className="px-8 py-5 text-center"><button onClick={() => setUseCustom(p => ({...p, [curr]: !p[curr]}))} className={`px-4 py-1.5 rounded-full text-[10px] font-black ${useCustom[curr] ? 'bg-orange-100 text-orange-600' : 'bg-green-100 text-green-600'}`}>{useCustom[curr] ? '手動' : '自動'}</button></td>
                    <td className="px-8 py-5 text-right"><input type="number" step="0.0001" disabled={!useCustom[curr]} value={customRates[curr] || ''} onChange={e => setCustomRates(p => ({...p, [curr]: parseFloat(e.target.value) || 0}))} className={`w-24 p-2 border rounded-xl text-sm text-right font-bold ${useCustom[curr] ? 'bg-white border-orange-300' : 'bg-slate-50 border-transparent'}`} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
    ...
    <div className="animate-fade-in space-y-8 w-full max-w-5xl mx-auto">
      <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-slate-100">
        <div className="flex justify-between items-end mb-4">
          <div><h3 className="text-2xl font-black text-slate-800">行李準備進度</h3><p className="text-sm font-bold text-slate-400">已整理 {completed} / {checklist.length} 件</p></div>
      ...
      <div className="bg-white p-10 rounded-[4rem] shadow-2xl border border-slate-100">
        <form onSubmit={async e => {
          ...
          await updateItinField('checklist', [...checklist, { id: Date.now().toString(), text: newItem.trim(), completed: false }]); setNewItem('');
        }} className="mb-10 flex gap-4"><input placeholder="新增必備物品..." value={newItem} onChange={e => setNewItem(e.target.value)} className="flex-1 p-5 bg-slate-50 border-2 border-transparent focus:border-blue-500 rounded-3xl outline-none font-bold shadow-inner" /><button type="submit" className="bg-blue-600 text-white px-8 rounded-3xl font-black shadow-lg"><Plus size={28}/></button></form>
          ...
          {checklist.map(item => (
            <div key={item.id} className={`p-6 rounded-[2rem] border flex items-center justify-between group transition-all ${item.completed ? 'bg-slate-50 opacity-60' : 'bg-white hover:border-blue-100'}`}>
              <div className="flex items-center gap-4 flex-1">
                <button onClick={async () => await updateItinField('checklist', checklist.map(i => i.id === item.id ? {...i, completed: !i.completed} : i))} className={`w-8 h-8 rounded-full flex items-center justify-center border-2 transition-all ${item.completed ? 'bg-green-500 border-green-500 text-white' : 'border-slate-200'}`}>{item.completed && <CheckCircle2 size={20} />}</button>
                {editingId === item.id ? <input autoFocus value={tempText} onChange={e => setTempText(e.target.value)} onBlur={async () => { await updateItinField('checklist', checklist.map(i => i.id === item.id ? {...i, text: tempText} : i)); setEditingId(null); }} className="bg-blue-50 px-2 py-1 rounded font-bold outline-none flex-1" /> : <span onClick={() => { setEditingId(item.id); setTempText(item.text); }} className={`font-bold cursor-text flex-1 ${item.completed ? 'line-through text-slate-400' : 'text-slate-700'}`}>{item.text}</span>}
              </div>
...
const App = () => {
  ...
  useEffect(() => {
    ...
    }, err => console.error("Snapshot Trips error", err));
  }, [user]);
  ...
  useEffect(() => {
    ...
    }, err => console.error("Snapshot Itinerary error", err));
