 ...
 * ------------------------------------------------
 * V8.1 最終穩定加固版：
 * 1. 版本標註：在登入首頁與主儀表板底部加入清晰的版本資訊。
 * 2. 訪客模式：支援透過 URL 參數 (?tripId=...&mode=view) 進入唯讀模式。
 * 3. 唯讀鎖定：訪客模式下隱藏所有「新增、修改、刪除、勾選」按鈕。
 * 4. 忠於原稿：完全還原 0215a.txt 的天氣、匯率、清單與費用組件。
 * 5. 帳號同步：Admin 進入後台自動補齊 abc 帳號至雲端。
 */

const VERSION_INFO = "旗艦終極穩定版 V8.1 - 2026/02/16 01:27";
  ...
  return (
    <div className="min-h-screen w-full bg-slate-50 flex items-center justify-center p-4 sm:p-6 font-sans text-slate-700 overflow-x-hidden">
      <div className="max-w-md w-full animate-fade-in text-center">
        ...
        {/* 💡 登入畫面底部的版本資訊 */}
        <p className="text-center text-slate-300 text-[10px] font-bold mt-12 uppercase tracking-widest leading-loose">Powered by 彥麟製作<br/>{VERSION_INFO}</p>
      </div>
    </div>
  );
};
...
const App = () => {
  ...
  return (
    <div className="w-full flex flex-col items-center min-h-screen text-slate-700">
      ...
      {view === 'home' ? (
        <div className="w-full max-w-5xl px-4 sm:px-6 py-10 sm:py-20 flex flex-col items-center animate-fade-in px-4">
        ...
        <div className="w-full flex flex-col items-center pb-24 animate-fade-in px-4">
          <nav className="w-full h-20 bg-white/90 backdrop-blur-xl border-b border-slate-100 flex items-center justify-between px-10 sticky top-0 z-50 shadow-sm max-w-[100vw] overflow-x-hidden">
            ...
            <div className="text-right flex items-center gap-4">
              <div className="hidden sm:block text-right"><div className="font-black text-slate-800 tracking-tight truncate max-w-[150px] leading-none">{tripInfo.city} 之旅</div><div className="text-[9px] font-bold text-slate-400 mt-1">{isGuestMode ? '訪客唯讀模式' : tripInfo.startDate}</div></div>
              {!isGuestMode && <button onClick={handleShare} className="p-2 text-blue-600 hover:bg-blue-50 rounded-xl transition-all shadow-sm bg-white" title="分享行程連結"><Share2 size={20}/></button>}
