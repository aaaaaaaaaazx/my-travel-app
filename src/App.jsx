...
import { 
  ...
  Wallet, Utensils, Home, Car, ShoppingBag, MoreHorizontal, Receipt, Snowflake, CloudRain, CloudLightning,
  TrendingUp, DollarSign, Settings2, RotateCcw, Search
} from 'lucide-react';

/**
 * 🏆 Travel Planner - 彥麟製作最終黃金基準旗艦版 (2026.02.07)
 * ------------------------------------------------
 * 1. 功能全恢復：恢復「自訂匯率模式」、「手動數值編輯」與「幣別搜尋」。
 * 2. 支出系統：完整保留分類記帳與統計儀表板。
 * 3. 天氣表單：恢復地點、日期範圍查詢與動態建議。
 * 4. 權限穩定：嚴格執行 Rule 1 & 3，徹底修復白屏與權限報錯。
 * 5. 媒體支援：行程備註支援圖片與自動超連結識別。
 */

const VERSION_INFO = "旗艦版 V3.0 - 2026/02/07 00:30";
...
const CHECKLIST_CATEGORIES = [
  { id: 'cat_3c', name: '3C 產品', icon: Smartphone, items: ['手機', '充電線', '充電器', '萬用轉接頭', '行動電源'] },
  { id: 'cat_clothing', name: '衣物', icon: Shirt, items: ['上衣', '褲子', '外套', '鞋子', '內衣褲'] },
  { id: 'cat_toiletries', name: '盥洗用品', icon: Bath, items: ['卸妝巾', '洗面乳', '牙膏', '牙刷', '濕紙巾'] },
  { id: 'cat_medicine', name: '個人藥品', icon: Pill, items: ['暈車藥', '過敏藥', '感冒藥', 'OK 繃'] },
  { id: 'cat_docs', name: '重要文件', icon: FileText, items: ['護照', '簽證', '國際駕照', '住宿憑證'] },
  { id: 'cat_others', name: '其他用品', icon: Package, items: ['水壺', '家鑰匙', '眼罩', '外幣現金', '頸枕'] }
];
    ...
    <div className="animate-fade-in space-y-10 w-full max-w-5xl mx-auto pb-10">
      <div className="bg-white p-8 md:p-12 rounded-[4rem] shadow-xl border border-slate-100">
        ...
        <form onSubmit={fetchWeather} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
          <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">目的地</label><input required value={q.dest} onChange={e => setQ({...q, dest: e.target.value})} className="w-full p-4 bg-slate-50 border-2 border-transparent focus:border-blue-500 rounded-3xl outline-none font-bold shadow-inner" /></div>
          <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">開始日期</label><input required type="date" value={q.start} onChange={e => setQ({...q, start: e.target.value})} className="w-full p-4 bg-slate-50 rounded-2xl font-bold outline-none shadow-inner" /></div>
          <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase ml-1">結束日期</label><input required type="date" value={q.end} min={q.start} onChange={e => setQ({...q, end: e.target.value})} className="w-full p-4 bg-slate-50 rounded-2xl font-bold outline-none shadow-inner" /></div>
      ...
      <div className="bg-white rounded-[3.5rem] shadow-2xl border border-slate-100 overflow-hidden p-8 md:p-14">
        <div className="grid grid-cols-1 md:grid-cols-7 gap-8 items-center">
          <div className="md:col-span-3">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block">輸入金額</label>
            <div className="relative">
              ...
              <div className="absolute right-4 top-1/2 -translate-y-1/2"><select value={baseCurrency} onChange={e => setBaseCurrency(e.target.value)} className="bg-white border rounded-xl px-2 py-1 text-xs font-black">{Object.keys(currencyNames).map(c => <option key={c} value={c}>{currencyNames[c]}</option>)}</select></div>
            </div>
          </div>
          ...
          <div className="md:col-span-3">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 block">轉換結果</label>
        ...
        <div className="mt-8 flex justify-between items-center">
            ...
            <button onClick={() => setShowSettings(!showSettings)} className="flex items-center gap-2 px-5 py-2 rounded-2xl text-xs font-black transition-all bg-white border text-slate-600 shadow-sm"><Settings2 size={16} /> 匯率管理與自訂</button>
        </div>
      </div>
      ...
      {showSettings && (
        <div className="bg-white rounded-[3rem] border border-slate-100 shadow-2xl overflow-hidden animate-fade-in">
          <div className="p-8 border-b flex justify-between items-center bg-slate-50/50">
              ...
              <div className="relative"><Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={18} /><input type="text" placeholder="搜尋幣別..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-12 pr-6 py-3 border rounded-2xl text-sm font-bold outline-none w-64" /></div>
          </div>
...
const App = () => {
  ...
  useEffect(() => {
    ...
    }, (err) => console.error("Itin error", err));
                ...
                <div className="bg-white p-8 md:p-12 rounded-[4rem] shadow-sm border border-slate-100">
                  <form onSubmit={async e => { 
                    ...
                    <div className="flex gap-3">
                      ...
                      <textarea placeholder="細節備註 (支援超連結)..." value={newSpot.note} onChange={e => setNewSpot({...newSpot, note: e.target.value})} className="flex-1 p-3 bg-white border rounded-xl font-medium h-20 resize-none text-sm shadow-sm" />
                        ...
                        <div key={item.id} className="relative pl-2 group">
                          ...
                          <div onClick={() => setExpandedItems(prev => ({...prev, [item.id]: !prev[item.id]}))} className={`p-10 bg-white border rounded-[3rem] transition-all cursor-pointer hover:shadow-2xl ${isExpanded ? 'border-blue-100 shadow-lg' : 'border-slate-100'}`}>
                            <div className="flex justify-between items-start gap-4">
                                    ...
                                    <div className="bg-slate-50/50 p-6 rounded-[2rem] border border-slate-100 space-y-4 animate-fade-in" onClick={e => e.stopPropagation()}>
                                      {item.imageUrl && (
                                        <div className="relative group/img overflow-hidden rounded-2xl border border-white shadow-sm max-w-md">
                                          <img src={item.imageUrl} alt={item.spot} className="w-full max-w-md h-auto rounded-2xl shadow-sm border border-white" onError={e => e.target.style.display='none'} onClick={(e) => { e.stopPropagation(); window.open(item.imageUrl, '_blank'); }} />
                                        </div>
                                      )}
                                ...
                                <button onClick={async (e) => { e.stopPropagation(); if(confirm('確定刪除？')) { const updated = itineraryData.days[activeDay].spots.filter(s => s.id !== item.id); await updateItinField(`days.${activeDay}.spots`, updated); } }} className="p-3 text-slate-200 hover:text-red-500 hover:bg-red-50 rounded-2xl transition-all"><Trash2 size={20}/></button>
                            </div>
                          </div>
                        </div>
                      );
